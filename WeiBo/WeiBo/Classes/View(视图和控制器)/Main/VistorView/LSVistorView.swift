...
class LSVistorView: UIView {
    ...
    //重写Set方法
    var vistorDicInfo: [String : String]?{
        didSet{
            //MARK: - 利用字典设置访客视图信息
            //使用字典设置信息
            //提示：如果是首页 imageName == ""
            //<1>取字典信息
            guard let imageName = vistorDicInfo?["imageName"],
                let   message = vistorDicInfo?["message"]  else {
                    return
            }
            //<2>设置消息
    ...
    //提示标签
    lazy var  tipLabel: UILabel = UILabel.cz_label(withText: "关注一些人，回这里看看有什么惊喜",
                                                   fontSize: 14,
...
extension LSVistorView{
    func setupUI() {
        ...
        addConstraint(NSLayoutConstraint(item: tipLabel, attribute: .centerY, relatedBy: .equal, toItem: iconView, attribute: .bottom, multiplier: 1.0, constant: margin))
        addConstraint(NSLayoutConstraint(item: tipLabel, attribute: .width, relatedBy: .equal, toItem: nil, attribute: .notAnAttribute, multiplier: 1.0, constant: 240))
        //<4>注册按钮
        ...
        //<6>遮罩头像 views:定义VFL 中的控件 名称和实际名称映射关系
